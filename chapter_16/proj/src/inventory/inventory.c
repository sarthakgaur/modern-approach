...
/*
 * sort: Sorts the inventory as items are inserted.
 */
void sort(void) {
    ...
    struct part current = inventory[num_parts-1];

    for (int i = 0; i < num_parts - 1; i++)
        if (current.number < inventory[i].number) {
            mark = i;
            insert_pos_found = true;
            break;
        }

    if (!insert_pos_found) return;

    for (int i = num_parts - 2; i >= mark; i--)
        inventory[i + 1] = inventory[i];

    inventory[mark] = current;
}
