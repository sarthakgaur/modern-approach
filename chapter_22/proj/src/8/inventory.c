...
/*
 * dump: Write the contents of the inventory to a file provided by
 * the user.
 */
void dump(void) {
    ...
    if (ferror(fp)) {
        fprintf(stderr, "Error: write operation on file %s was not successful\n", filename);
        exit(EXIT_FAILURE);
    }
...
/*
 * restore: Read the contents of the inventory from a file provided by
 * the user.
 */
void restore(void) {
    char filename[MAX_FILENAME + 1];
    ...
    n = fread(inventory, sizeof(inventory[0]), sizeof(inventory) / sizeof(inventory[0]), fp);
    num_parts = n;

    if (ferror(fp)) {
        fprintf(stderr, "Error: read operation on file %s was not successful\n", filename);
        exit(EXIT_FAILURE);
    }
